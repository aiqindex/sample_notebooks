...
def filter_to_latest_releases(
    ...
    df.set_index(['Ticker', 'Date', 'variable', 'SMOOTH', 'release_timestamp'], inplace=True)
    df = df.xs(0, level='SMOOTH').drop(['backfill'], axis=1)
    df = df.sort_index()
    df.reset_index('release_timestamp', drop=False, inplace=True)
    df = df.loc[~df.index.duplicated(keep='last')]
    df.drop(['release_timestamp'], axis=1, inplace=True)
    return df.unstack('variable')['values']
    

def load_retail(sdh: DAL) -> int:
    path_to_csmr_retailer = os.path.join(
        DEFAULT_DIR,
        'aiq_pos_evenly/20240312_pos_retailer_stack.parquet')
    dfpos_retailer = pd.read_parquet(path_to_csmr_retailer)
    dfpos_retailer = filter_to_latest_releases(dfpos_retailer)
    dfpos_retailer.index.names = ['ticker', 'datetime']
    id = sdh.set_raw_data(
        dfraw=dfpos_retailer, data_source='aiQ', source='pos_retailer')
    return id


def load_goods(sdh: DAL) -> int:
    path_to_csmr_good_gen1 = os.path.join(
        DEFAULT_DIR,
        'aiq_pos_evenly/20240312_pos_csmr_goods_stack_001.parquet')
    df_inc1 = pd.read_parquet(path_to_csmr_good_gen1)
    df_inc1 = filter_to_latest_releases(df_inc1)
    df_inc1

    path_to_csmr_good_gen2 = os.path.join(
        DEFAULT_DIR,
        'aiq_pos_evenly/20240312_pos_csmr_goods_stack_002.parquet')
    df_inc2 = pd.read_parquet(path_to_csmr_good_gen2)
    df_inc2 = filter_to_latest_releases(df_inc2)
    df_inc2

    dfpos_goods = pd.concat(
        [df_inc1.loc[~df_inc1.index.isin(df_inc2.index)], df_inc2], axis=0)
    dfpos_goods.sort_index(inplace=True)
    id = dfpos_goods.sort_index(inplace=True)
    dfpos_goods.index.names = ['ticker', 'datetime']
    return id
...
def format_pos(df):
    ...
    df.set_index(
        ['Ticker', 'Date', 'VARIABLE', 'SMOOTH', 'RELEASE_TIMESTAMP'],
        inplace=True)
    df = df.xs(0, level='SMOOTH').drop(['BACKFILL'], axis=1)
    df = df.sort_index()
    df.reset_index('RELEASE_TIMESTAMP', drop=False, inplace=True)
...
def load_elec(sdh: DAL) -> int:
    path_to_csmr_elec = os.path.join(
        DEFAULT_DIR, 'aiq_pos_evenly/pos_elec.parquet')
    dfpos_elec = pd.read_parquet(path_to_csmr_elec)
    dfpos_elec = format_pos(dfpos_elec)
    dfpos_elec.index.names = ['ticker', 'datetime']
    id = sdh.set_raw_data(
        dfraw=dfpos_elec, data_source='aiQ', source='pos_elec')
    return id


def load_market(sdh: DAL) -> int:
    path_to_stockprice = os.path.join(
        DEFAULT_DIR, 'aiq_pos_evenly/stock_price_daily_202403.parquet')
    pricedf = pd.read_parquet(path_to_stockprice)
    pricedf.index.names = ['ticker', 'datetime']
    id = sdh.set_raw_data(dfraw=pricedf, data_source='aiQ', source='market')
    return id
